...
function SplitText({ text, className, stroke = false, onClick }: { text: string, className?: string, stroke?: boolean, onClick?: () => void }) {
  return (
    <motion.h1 
      initial="hidden"
      animate="visible"
...
function AnimatedName({ text, stroke = false, onClick, direction }: { text: string, stroke?: boolean, onClick?: () => void, direction: number }) {
  return (
    <motion.h1 
      custom={direction}
...
export function HeroSolid() {
  ...
  const paddedName = displayName === "JOHN" ? `JOHN${ZERO_WIDTH_SPACE}` : "ANGEL";
